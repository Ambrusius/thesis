...
def main(rank, ws):
    ...
    while len(avail_ws) > 0:
        ...
        comm.send(sample_small.iloc[job_index], dest=avail_ws[-1], tag=11)
    ...
    while len(avail_tasks) > 0:
        ...
        if len(avail_tasks) > 0:
            job_index = avail_tasks[-bunch_size:]
            comm.send(sample_small.iloc[job_index], dest=w, tag=11)
            task_ws[w] = job_index
            avail_tasks = avail_tasks[:-bunch_size]
        else:
...
def worker(rank, ws):
    ...
    state = MPI.Status()
    while True:
        small_sample = comm.recv(source=0, tag=MPI.ANY_TAG, status= state)
        if state.Get_tag() == 10:
            break
        new_rows = job(small_sample)
        comm.send(new_rows, dest=0, tag=11)
